...
// Org-scoped named secret
const sirr2 = new SirrClient({ token: process.env.SIRR_TOKEN!, org: process.env.SIRR_ORG! })
await sirr2.set('API_KEY', 'sk-...', { ttl: 3600, reads: 1 })
const value = await sirr2.get('API_KEY')   // null if burned or expired
```
...
```bash {{ title: 'npx' }}
npx @sirrlock/sirr push "postgres://..." --reads 1 --ttl 1h
npx @sirrlock/sirr set DB_URL="postgres://..." --org $SIRR_ORG --reads 1 --ttl 1h
npx @sirrlock/sirr get DB_URL --org $SIRR_ORG
```
