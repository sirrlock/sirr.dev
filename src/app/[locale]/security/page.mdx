...
Secret values are never included in list responses. To retrieve a value, you must explicitly call `GET /secrets/:id` (public) or `GET /orgs/:org_id/secrets/:key` (org-scoped), which increments the read counter and may destroy the secret if the read limit is reached.
