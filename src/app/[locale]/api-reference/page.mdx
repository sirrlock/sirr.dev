...
export const sections = [
  ...
  { title: 'Update a secret', id: 'patch-secrets-key' },
...
<Row>
  ...
  <Col sticky>
    ...
    ```js {{ title: 'Node.js' }}
    const response = await fetch('http://localhost:39999/secrets/DB_URL', {
      headers: {
        'Authorization': `Bearer ${process.env.SIRR_MASTER_KEY}`,
      },
    })
    ...
    response = requests.get(
        "http://localhost:39999/secrets/DB_URL",
        headers={"Authorization": f"Bearer {SIRR_MASTER_KEY}"},
    )
...
## Update a secret {{ tag: 'PATCH', label: '/secrets/:key', id: 'patch-secrets-key' }}
...
<Row>
  ...
  <Col sticky>
    ...
    <CodeGroup title="Request" tag="PATCH" label="/secrets/:key">
