...
function PageNavigation() {
  ...
  const { t } = useLocale()
  let allPages = navGroups.flatMap((group) => group.links)
    ...
    <div className="flex">
      ...
      {nextPage && (
        <div className="ml-auto flex flex-col items-end gap-3">
          <PageLink label={t('common.next')} page={nextPage} />
        </div>
      )}
    </div>
  )
  ...
  return (
    <div className="flex flex-col items-center justify-between gap-5 border-t border-zinc-900/5 pt-8 sm:flex-row dark:border-white/5">
      <p className="text-xs text-zinc-600 dark:text-zinc-400">
        &copy; {t('common.copyright', { year: new Date().getFullYear() })}
      </p>
      <div className="flex gap-4">
        <SocialLink href="https://github.com/SirrVault" icon={GitHubIcon}>
          {t('common.followOnGithub')}
        </SocialLink>
      </div>
    </div>
  )
